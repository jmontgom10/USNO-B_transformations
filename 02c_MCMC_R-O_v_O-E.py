...
goodStars = np.logical_not(landoltStars['Omag'].data.mask)
goodStars = np.logical_and(
    goodStars,
    np.logical_not(landoltStars['Emag'].data.mask))
goodStars = np.logical_and(
    goodStars,
    np.isfinite(landoltStars['Omag'].data))
goodStars = np.logical_and(
    goodStars,
    np.isfinite(landoltStars['Emag'].data))
...
Rmags    = Vmags - landoltStars['V-R'].data.data
sRmags   = np.sqrt(sVmags**2 + landoltStars['e_V-R'].data.data**2)
...
bounds = [(-0.62, -0.42),   # Theta (angle of the line slope)
          (-0.25, 0.15),    # b_perp (min-dist(line-origin))
...
plt.axis((-2.0, 5.0, -4.3, 2.1))
plt.autoscale(False)

# Plot the slope +/- sigma_intrinsic lines
# Start by including a "typical errorbar"
plt.errorbar(-1.1, 3.9, xerr=data[2], yerr=data[3], color='k')
