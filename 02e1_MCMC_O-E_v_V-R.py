...
bounds = [(0.5, 2.2),       # Theta (angle of the line slope)
          (-0.64, 0.36),    # b_perp (min-dist(line-origin))
...
plt.axis((-0.6, 1.6, -2.0, 5.0))
plt.autoscale(False)

# Plot the slope +/- sigma_intrinsic lines
# Start by including a "typical errorbar"
plt.errorbar(-0.4, 4.0, xerr=np.median(data[2]), yerr=data[3], color='k')
